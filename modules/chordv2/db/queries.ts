...
// All reads here go against chord_qualities / chord_shapes / open_chords.
// In practice these only change via direct SQL (bulk seed scripts, manual
// edits) — createNewChordShape (app/chord/actions.ts) calls
// updateTag('chord-data') for the rare in-app write, but that's not the
// primary freshness mechanism. A long revalidate is: it means a direct SQL
// edit surfaces on its own within a month with no manual step required.
const CHORD_DATA_TAGS = ['chord-data'];
const CHORD_DATA_REVALIDATE = 60 * 60 * 24 * 30; // 30 days

export const getQualityBySymbol = unstable_cache(
  async (symbol: string): Promise<DbChordQuality | null> => {
...
export const getAllQualities = unstable_cache(
  async (): Promise<DbChordQuality[]> => {
    const { rows } = await sql.query<DbChordQuality>(
      `SELECT id, symbol, full_name, intervals, degrees, category, interval_overrides, aliases
...
export const getShapesBySymbol = unstable_cache(
  async (symbol: string): Promise<DbChordShape[]> => {
...
export const getInversionShapes = unstable_cache(
  async (symbol: string, bassString: number): Promise<DbChordShape[]> => {
    const { rows } = await sql.query<
      Omit<DbChordShape, 'tab_relative'> & { tab_relative: (number | null)[] }
    >(
      `SELECT cs.id, cs.quality_id, cq.symbol AS quality_symbol, cq.full_name AS quality_full_name,
...
export const getCanonicalShapePerQuality = unstable_cache(
  async (
...
export const getShapesAtPosition = unstable_cache(
  async (
    rootString: number,
...
export const getFixedInversionsForRoot = unstable_cache(
  async (symbol: string, rootPc: number): Promise<DbChordShape[]> => {
    const { rows } = await sql.query<
      Omit<DbChordShape, 'tab_relative'> & { tab_relative: (number | null)[] }
    >(
      `SELECT cs.id, cs.quality_id, cq.symbol AS quality_symbol, cq.full_name AS quality_full_name,
...
export const getAvailableFingersForRoot = unstable_cache(
  async (rootString: number, rootPc: number): Promise<number[]> => {
...
export const getFingerZeroHasStretchShapes = unstable_cache(
  async (rootString: number): Promise<boolean> => {
...
export const isOpenChord = unstable_cache(
  async (chordShapeId: number, rootPc: number): Promise<boolean> => {
    const { rows } = await sql.query<{ exists: boolean }>(
      `SELECT EXISTS(
...
export const getAllOpenChordShapes = unstable_cache(
  async (): Promise<OpenChordShape[]> => {
    const { rows } = await sql.query<
...
export const getAllSlashChordShapes = unstable_cache(
  async (): Promise<DbChordShape[]> => {
    const { rows } = await sql.query<
      Omit<DbChordShape, 'tab_relative'> & { tab_relative: (number | null)[] }
    >(
      `SELECT cs.id, cs.quality_id, cq.symbol AS quality_symbol, cq.full_name AS quality_full_name,
...
export const getShapesBySymbols = unstable_cache(
  async (symbols: string[], rootStrings: number[]): Promise<DbChordShape[]> => {
...
export const getNonMoveableShapesByRootPcs = unstable_cache(
  async (rootPcs: number[]): Promise<DbChordShape[]> => {
...
export const getShapesForRelated = unstable_cache(
  async (qualityIds: number[], rootString: number): Promise<DbChordShape[]> => {
